...
public class Index implements Serializable, Closeable {
    ...
    IndexOne2N names;
    IndexOne2One busNums;
    IndexOne2N routeNums;

    public void test(Bus bus) throws KeyNotUniqueException {
        assert (bus != null);
        //check requirements for unique keys
        if (busNums.contains(Integer.toString(bus.busNum))) {
            throw new KeyNotUniqueException(Integer.toString(bus.busNum));
        }
    }

    public void put(Bus bus, long value) throws KeyNotUniqueException {
        test(bus);
        busNums.put(Integer.toString(bus.busNum), value);
        routeNums.put(Integer.toString(bus.routeNum), Bus.authorDel, value);
        names.put(bus.name, value);
    }

    public Index() {
        busNums = new IndexOne2One();
        routeNums = new IndexOne2N();
